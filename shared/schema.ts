...
export const users = pgTable("users", {
  ...
  isAI: boolean("is_ai").default(false), // Mark AI accounts
...
export const messages = pgTable("messages", {
  ...
  mediaUrls: json("media_urls").$type<Array<{ type: string; url: string; thumbnailUrl?: string }>>(),
  createdAt: timestamp("created_at").defaultNow(),
...
export const notifications = pgTable("notifications", {
  ...
  type: varchar("type", { length: 50 }).notNull(), // "like", "comment", "suggestion", "violation", "admin_announcement", "admin_message"
  title: text("title").notNull(),
  ...
  data: jsonb("data").$type<{
    ...
    senderId?: number; // for admin messages
    priority?: "low" | "medium" | "high"; // for admin announcements
  }>().default({}),
