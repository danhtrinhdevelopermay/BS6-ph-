...
import { Heart, MessageCircle, Upload, Image, Video, Send, MoreVertical, Plus, Share, Flag, Trash2 } from "lucide-react";
...
export default function Posts() {
  ...
  const [selectedMediaFiles, setSelectedMediaFiles] = useState<Array<{
    type: "image" | "video";
    url: string;
    thumbnailUrl?: string;
    file: File;
  ...
  const createPostMutation = useMutation({
    mutationFn: async (data: PostFormData) => {
      ...
      const postData: InsertPost = {
        ...
        mediaUrls: selectedMediaFiles.map(media => ({
          type: media.type,
          url: media.url,
  ...
  const reportPostMutation = useMutation({
    mutationFn: async ({ postId, reason }: { postId: number; reason: string }) => {
      ...
      return apiRequest("POST", `/api/posts/${postId}/report`, reportData);
    },
    onSuccess: () => {
  ...
  const handleSharePost = (postId: number) => {
    ...
    if (navigator.share) {
      navigator.share({
        ...
        text: "Check out this post on BSC",
        url: shareUrl,
      });
    } else {
  ...
  const submitReport = () => {
    ...
    reportPostMutation.mutate({ postId: reportingPostId, reason: reportReason });
  };
  ...
  const handleMediaClick = (media: { url: string; type: string; thumbnailUrl?: string }, e: React.MouseEvent) => {
    e.stopPropagation();
    ...
    <div className="container mx-auto px-4 py-6 max-w-2xl pb-20">
      ...
      <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
        <DialogTrigger asChild>
          <Button
            className="fixed bottom-28 right-6 h-14 w-14 rounded-full shadow-lg hover:shadow-xl transition-all duration-300 bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 border-0 z-50"
            size="icon"
          >
        ...
        <DialogContent className="sm:max-w-[525px]">
          ...
          <div className="space-y-4">
            ...
            />
            <MediaUpload
              selectedMedia={selectedMediaFiles}
              onMediaChange={setSelectedMediaFiles}
            />
      ...
      {selectedMediaModal && (
        <MediaModal
          ...
          mediaUrl={selectedMediaModal.url}
          mediaType={selectedMediaModal.type}
          thumbnailUrl={selectedMediaModal.thumbnailUrl}
        />
      )}
    </div>
  );
    ...
    return (
      <Card>
        ...
        <CardContent className="space-y-4">
          <p className="text-sm leading-relaxed cursor-pointer hover:bg-muted/50 p-2 rounded-md transition-colors"
          ...
          {post.mediaUrls && post.mediaUrls.length > 0 && (
            <div className="grid gap-2">
              {post.mediaUrls.map((media, index) => (
                <div key={index} className="rounded-lg overflow-hidden hover:opacity-90 transition-opacity cursor-pointer">
                  {media.type === "image" ? (
                    <img 
                      src={media.url} 
                      alt="Post media" 
