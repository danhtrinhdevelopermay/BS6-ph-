...
export interface IStorage {
  ...
  sendMessage(conversationId: number, senderId: number, content: string, type?: string, mediaUrls?: Array<{ type: string; url: string; thumbnailUrl?: string }>): Promise<Message>;
...
export class DatabaseStorage implements IStorage {
  ...
  async getConversationMessages(conversationId: number, limit = 50): Promise<Message[]> {
    try {
      ...
      // If mediaUrls column doesn't exist, fall back to basic query
      const results = await db.select({
        id: messages.id,
  ...
  async sendMessage(conversationId: number, senderId: number, content: string, type = 'text', mediaUrls?: Array<{ type: string; url: string; thumbnailUrl?: string }>): Promise<Message> {
    try {
      // Process media URLs to optimize storage
      let processedMediaUrls = mediaUrls;
      if (mediaUrls && mediaUrls.length > 0) {
        try {
          // Use cloud media service if available
          const { cloudMedia } = await import('./cloud-media');
          processedMediaUrls = await cloudMedia.processMediaUrls(mediaUrls);
        } catch (error) {
          // If cloud media fails, keep original URLs
          console.warn('Cloud media processing failed, using original URLs:', error);
        }
      }
      ...
      const [message] = await db.insert(messages).values({
        ...
        content,
        type,
        mediaUrls: processedMediaUrls || null,
      }).returning();
      ...
      // If mediaUrls column doesn't exist, fall back to basic insert
      const [message] = await db.insert(messages).values({
        conversationId,
        senderId,
        content,
  ...
  // Posts methods implementation with performance optimization
  async getPosts(limit = 20, offset = 0): Promise<(Post & { user: User; hasLiked?: boolean; likesCount: number; commentsCount: number })[]> {
    ...
    const results = await db
      .select({
        ...
        // Select only essential user fields to reduce data transfer
        username: users.username,
        displayName: users.displayName,
      ...
      .limit(Math.min(limit, 50)) // Cap limit to prevent excessive data
      .offset(offset);
