...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/ai/profile", async (req, res) => {
    try {
      const aiProfile = await aiSystem.getAIProfile();
      res.json(aiProfile);
    } catch (error) {
  ...
  // Initialize AI system endpoint (for testing)
  app.post("/api/ai/initialize", async (req, res) => {
    try {
      const aiUser = await aiSystem.getAIUser();
      res.json({ message: "AI initialized successfully", aiUser });
    } catch (error) {
      res.status(500).json({ message: "Failed to initialize AI" });
    }
  });
  ...
  app.get("/api/users/username/:username", async (req, res) => {
    try {
      ...
      // If it's the AI user, return enhanced profile
      if (user.isAI) {
        const aiProfile = await aiSystem.getAIProfile();
        res.json(aiProfile);
      } else {
  ...
  app.post("/api/posts", async (req, res) => {
    try {
      ...
      // Trigger AI response for the new post (async, don't wait)
      if (post && post.content) {
        aiSystem.processNewPost(post.id, post.content).catch(error => {
          console.error('AI processing failed for post:', post.id, error);
        });
      }
